...
set(ABI_GEN_PATH ${EOSIO_INSTALL_DIR}/bin/eosio-abigen)
set(EOSIO_CONTRACTSDK_PATH /usr/local/usr/share/eosio/contractsdk)
set(EOSIO_LIB_PATH ${EOSIO_CONTRACTSDK_PATH}/lib)
...
macro(compile_wast)
    ...
    foreach (srcfile ${SOURCE_FILES})
        ...
        endforeach ()
        foreach (folder ${INCLUDE_DIRECTORIES})
            list(APPEND WASM_COMMAND -I ${folder})
        endforeach ()

        list(APPEND WASM_COMMAND -I ${EOSIO_INSTALL_DIR}/include)
        list(APPEND WASM_COMMAND -I ${EOSIO_INSTALL_DIR}/include/libc++/upstream/include)
        list(APPEND WASM_COMMAND -I ${EOSIO_INSTALL_DIR}/include/musl/upstream/include)
        list(APPEND WASM_COMMAND -I include)
        list(APPEND WASM_COMMAND -I /usr/local/Cellar/boost/1.67.0_1/include)
...
macro(add_wast_executable)
    ...
    foreach (lib ${ARG_LIBRARIES})
        list(APPEND LIBRARIES ${EOSIO_LIB_PATH}/${lib}.bc)
    endforeach ()
    ...
    add_custom_command(OUTPUT ${DESTINATION_FOLDER}/${target}.wast
            DEPENDS ${target}.s
            COMMAND ${S2WASM_PATH} -o ${DESTINATION_FOLDER}/${target}.wast -s 10240 ${MAX_MEMORY_PARAM} ${target}.s
    ...
    add_custom_command(OUTPUT ${DESTINATION_FOLDER}/${target}.wasm
            DEPENDS ${target}.wast
            COMMAND ${WAST2WASM_PATH} ${DESTINATION_FOLDER}/${target}.wast ${DESTINATION_FOLDER}/${target}.wasm -n
